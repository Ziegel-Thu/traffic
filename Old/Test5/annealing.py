...
class SimulatedAnnealingVRPPD:
    def __init__(self, vrppd):
        self.vrppd = vrppd
        self.current_solution = vrppd.routes.copy()
        self.best_solution = vrppd.routes.copy()
        self.best_cost = self.calculate_total_cost(vrppd.routes)
        self.temperature = 1000.0  # Initial temperature
        self.cooling_rate = 0.9999  # Cooling rate
        self.iterations = 100000  # Maximum iterations

    def generate_neighbor_solution(self):
        new_solution = [route.copy() for route in self.current_solution]
        # Randomly select two different vehicles
        vehicle1, vehicle2 = random.sample(range(self.vrppd.num_vehicles), 2)
        
        # Randomly select indices within the route of each vehicle
        index1 = random.randint(0, len(new_solution[vehicle1]) - 1)
        index2 = random.randint(0, len(new_solution[vehicle2]) - 1)
        
        # Ensure selected indices are valid pickup points
        while new_solution[vehicle1][index1] > self.vrppd.num_customers or new_solution[vehicle2][index2] > self.vrppd.num_customers:
            index1 = random.randint(0, len(new_solution[vehicle1]) - 1)
            index2 = random.randint(0, len(new_solution[vehicle2]) - 1)
        
        # Swap the pickups and their corresponding deliveries
        customer1_pickup = new_solution[vehicle1][index1]
        customer1_delivery = customer1_pickup + self.vrppd.num_customers
        customer2_pickup = new_solution[vehicle2][index2]
        customer2_delivery = customer2_pickup + self.vrppd.num_customers
        
        for i in range(len(new_solution[vehicle1])):
            if new_solution[vehicle1][i] == customer1_delivery:
                new_solution[vehicle1][i] = customer2_delivery
        
        for i in range(len(new_solution[vehicle2])):
            if new_solution[vehicle2][i] == customer2_delivery:
                new_solution[vehicle2][i] = customer1_delivery
        
        new_solution[vehicle1][index1] = customer2_pickup
        new_solution[vehicle2][index2] = customer1_pickup
    ...
    def calculate_total_cost(self, solution):
        return self.vrppd.total_distance(solution) + self.vrppd.total_penalty(solution)

    def optimize(self):
        current_cost = self.best_cost

        for _ in range(self.iterations):
            new_solution = self.generate_neighbor_solution()
            new_cost = self.calculate_total_cost(new_solution)

            if self.acceptance_probability(current_cost, new_cost, self.temperature) > random.random():
...
def main():
    ...
    sa_vrppd = SimulatedAnnealingVRPPD(vrppd)
    best_solution = sa_vrppd.optimize()
    print("优化后的解：")
    print_solution(best_solution, vrppd)
    print("优化后总成本：", vrppd.total_distance(best_solution) + vrppd.total_penalty(best_solution))
