...
class VRPPD:
    ...
    def neighbor(self, routes):
        ...
        if len(new_routes[route1]) > 0 and len(new_routes[route2]) > 0:
            ...
            load1 = sum(self.demands[customer - 1] for customer in new_routes[route1] if customer != 0 and customer != 2 * self.num_customers + 1)
            load2 = sum(self.demands[customer - 1] for customer in new_routes[route2] if customer != 0 and customer != 2 * self.num_customers + 1)
    ...
    def simulated_annealing(self, initial_temp, cooling_rate, stopping_temp):
        ...
        # Ensure each route ends with the final depot after simulated annealing
        final_depot = 2 * self.num_customers + 1
        for i in range(self.num_vehicles):
            if best_solution[i] and best_solution[i][-1] != final_depot:
                best_solution[i].append(final_depot)
