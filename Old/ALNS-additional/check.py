...
def validate_solution(solution, demand, vehicle_capacity, N, window, dis):
    valid = True
    
    for vehicle_idx, route in enumerate(solution):
        print(f"\nChecking vehicle {vehicle_idx + 1} route: {route}")

        if route[0] != 0 or route[-1] != 13:
            print(f"Vehicle {vehicle_idx + 1}: Path must start at 0 and end at 13.")
            valid = False
        ...
        for idx in range(1, len(route)):
            ...
            # 更新时间
            current_time += dis[prev_node][node]

            if node <= N:  # Pickup node
                current_load += demand[node - 1]
                if current_load > vehicle_capacity:
                    print(f"Vehicle {vehicle_idx + 1}: Exceeded vehicle capacity at node {node}.")
                    valid = False
                visited_nodes.add(node)
            elif node<=2*N:

                pickup_node = node - N
                if pickup_node not in visited_nodes:
                    print(f"Vehicle {vehicle_idx + 1}: Delivery at node {node} without corresponding pickup.")
                    valid = False
                current_load -= demand[pickup_node - 1]

            # 检查时间窗口，计算超时惩罚
            if current_time > window[node-1]:
                penalty = current_time - window[node-1]
                print(f"Vehicle {vehicle_idx + 1}: Node {node} exceeds time window by {penalty:.8f}. Penalty applied,penalty {e*penalty:.8f}")

            print(f"Vehicle {vehicle_idx + 1}: Node {node} reached at time {current_time:.8f} with load {current_load}.")
...
if __name__ == '__main__':
    ...
    if model.status == GRB.OPTIMAL:
        ...
        # 在这里增加验证代码
    solution = [[0,1,5,7,11,2,8,13],[0,3,9,6,4,10,12,13],[0,13],[0,13],[0,13]]
    
    validate_solution(solution, demand, 100, N, window, dis)
