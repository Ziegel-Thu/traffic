    ...
    def initial_solution(self):
        ...
        while remaining_customers or any(load > 0 for load in vehicle_load):
            for vehicle in range(self.num_vehicles):
                ...
                # Check for the best pickup or delivery that minimizes cost
                for customer in remaining_customers:
                    if vehicle_load[vehicle] + self.demands[customer - 1] <= self.vehicle_capacity:
                        temp_route = routes[vehicle] + [customer]
                        temp_cost = self.calculate_route_distance(temp_route) + self.calculate_route_penalty(temp_route)-self.calculate_route_distance(routes[vehicle])-self.calculate_route_penalty(routes[vehicle])
                ...
                for delivery in remaining_deliveries:
                    if (delivery - self.num_customers) in routes[vehicle]:
                        temp_route = routes[vehicle] + [delivery]
                        temp_cost = self.calculate_route_distance(temp_route) + self.calculate_route_penalty(temp_route)-self.calculate_route_distance(routes[vehicle])-self.calculate_route_penalty(routes[vehicle])
                        if temp_cost < best_cost:
                            best_cost = temp_cost
                            best_customer = None
...
def ALNS(vrppd, max_iterations=1000, destruction_level=0.3, initial_temperature=100.0, cooling_rate=0.99):
